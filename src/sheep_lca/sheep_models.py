    ...
    def __init__(self, data):
        ...
        self.grasses["average"] = {
            "forage_dry_matter_digestibility": self.average(
                self.data_frame, "forage_dry_matter_digestibility"
            ),
    ...
    def __init__(self, data):
        ...
        self.concentrates["average"] = {
            "con_dry_matter_digestibility": self.average(
                self.data_frame, "con_dry_matter_digestibility"
            ),
...
def load_farm_data(**kwargs):

    farm_path = kwargs.get("farm_path")
    animal_path = kwargs.get("animal_path")

    if farm_path is None or animal_path is None:
        raise Exception("No data paths provided ... ")

    # 1. Load each animal category into an object

    animal_data_frame = pandas.read_csv(animal_path)
    ...
    farm_data_frame = pandas.read_csv(farm_path)
...
def load_excel_file(data_path):

    pandas.read_excel(data_path, sheet_name="CP")
